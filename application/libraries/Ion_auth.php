...
class Ion_auth
{
	...
	// Scout Admin (ID 2)
	public function is_scout_admin($id=false)
	{
		$this->ion_auth_model->trigger_events('is_scout_admin');
		$group = $this->config->item('scout_admin_group', 'ion_auth');
		return $this->in_group($group, $id);
	}

	// Monitoring Team (ID 3) is_monitor_team
	public function is_monitor_team_admin($id=false)
	{
		$this->ion_auth_model->trigger_events('is_monitor_team_admin');
		$group = $this->config->item('monitor_team_group', 'ion_auth');
		return $this->in_group($group, $id);
	}

	// Regional Head (ID 4) is_regional_head/is_region_group
	public function is_region_admin($id=false)
	{
		$this->ion_auth_model->trigger_events('is_region_admin');
		$group = $this->config->item('region_group', 'ion_auth');
		return $this->in_group($group, $id);
	}

	// District Office (ID 5) is_district_office/is_district_group
	public function is_district_admin($id=false)
	{
		$this->ion_auth_model->trigger_events('is_district_admin');
		$group = $this->config->item('district_group', 'ion_auth');
		return $this->in_group($group, $id);
	}	

	// Upazilla Office (ID 6) is_upa_tha_office/is_upazila_group
	public function is_upazila_admin($id=false)
	{
		$this->ion_auth_model->trigger_events('is_upazila_admin');
		$group = $this->config->item('upazila_group', 'ion_auth');
		return $this->in_group($group, $id);
	}

	// Unit Leader (ID 7) is_scout_group
	public function is_group_admin($id=false)
	{
		$this->ion_auth_model->trigger_events('is_group_admin');
		$group = $this->config->item('group_leader_group', 'ion_auth');
		return $this->in_group($group, $id);
	}

	// Scout Accountant (ID 8) is_unit_group
	public function is_unit_admin($id=false)
	{
		$this->ion_auth_model->trigger_events('is_unit_admin');
		$group = $this->config->item('unit_leader_group', 'ion_auth');
		return $this->in_group($group, $id);
	}

	// Scout Member (ID 9)
	public function is_scout_member($id=false)
	{
		$this->ion_auth_model->trigger_events('is_scout_member');
		$group = $this->config->item('member_group', 'ion_auth');
		return $this->in_group($group, $id);
	}

	// Guest Member (ID 10)
	public function is_guest($id=false)
	{
		$this->ion_auth_model->trigger_events('is_guest');
	...
	{
		$this->ion_auth_model->trigger_events('is_vendor');
		$group = $this->config->item('vendor_group', 'ion_auth');
		return $this->in_group($group, $id);
	}

   // Guest Member (ID 12)
   public function is_employee($id=false)
   {
      $this->ion_auth_model->trigger_events('is_employee');
      $group = $this->config->item('employee_group', 'ion_auth');
      return $this->in_group($group, $id);
   }
