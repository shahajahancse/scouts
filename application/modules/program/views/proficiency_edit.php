                  ...
                  <div class="col-md-12">
                     <div class="row form-row">
                        <div class="col-md-4">
                           ...
                           echo form_dropdown('badge_id', $badges, set_value('badge_id', $infoProficiency->badge_id), $more_attr);
                           ?>
                        </div>
                        <div class="col-md-4">
                           ...
                           echo form_dropdown('prof_badge_group_id', $prof_badge_group, set_value('prof_badge_group_id', $infoProficiency->prof_badge_group_id), $more_attr);
                           ?>
                        </div>
                        <div class="col-md-4">
                           <label class="form-label">Proficiency Badge Name<span class="required">*</span></label>
                           <?php echo form_error('prof_badge_id');
                           $more_attr = 'class="proficiency_badge_val form-control input-sm"';
                           echo form_dropdown('prof_badge_id', $prof_badge_name_list, set_value('prof_badge_id', $infoProficiency->prof_badge_id), $more_attr);
                           ?>
                        </div>
                     </div>
                     ...
                     <div class="row form-row">
                        <div class="col-md-3">  
                           ...
                           <input type="text" name="achieved_date" class="form-control input-sm datetime" value="<?=set_value('achieved_date', date_browse_format($infoProficiency->achieved_date))?>" placeholder="DD-MM-YYYY">
                        </div>
                        <div class="col-md-2">
                           <label class="form-label">Extra Badge</label>
                           <?php echo form_error('extra_badge'); 
                           $more_attr = 'class="form-control input-sm"';
                           echo form_dropdown('extra_badge', $extra_badge, set_value('extra_badge', $infoProficiency->extra_badge), $more_attr);
                           ?>
                        </div>
                        <div class="col-md-4">
                           <label class="form-label">Evaluated By</label>
                           <?php echo form_error('evaluated_by');?>
                           <select class="scoutIDSingleSelect2 form-control" name="evaluated_by" id="evaluated_by_id" style="width:100%"></select>
                           <script>
                              var $newOption = $("<option></option>").val("<?php echo $infoProficiency->evaluated_by;?>").text("<?php echo $infoProficiency->scout_id.' ('.$infoProficiency->first_name.')'?>");
                              $("#evaluated_by_id").append($newOption).trigger('change');
                           </script>
                        </div>
                     </div>
