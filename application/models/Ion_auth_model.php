...
class Ion_auth_model extends CI_Model
{
	...
	public function forget_change_password($identity, $new)
	{
		$this->trigger_events('pre_change_password');
		...
		if (!empty($user->id))
		{
			...
			$successfully_changed_password_in_db = $this->db->update($this->tables['users'], $data, array('id' => $identity));
			if ($successfully_changed_password_in_db)
			{
				$this->trigger_events(array('post_change_password', 'post_change_password_successful'));
				$this->set_message('password_change_successful');
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, is_office, scout_id, active, last_login, first_name, last_name, created_on')
		                  ->where($this->identity_column, $identity)
		                  ->or_where('scout_id', $identity)
		                  ->limit(1)
		    			  		->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	/**
	 * login for Android Application
	 *
	 * @return array
	 * @author Ataul Mostafa
	 **/
	public function login_apps($identity, $password, $remember=FALSE)
	{
		$this->trigger_events('pre_login');
		...
		$query = $this->db->select($this->identity_column . ', email, id, is_office, scout_id, active, last_login, first_name, created_on, profile_img, is_request')
		    			  		->where('is_office', 0)
		                  ->where($this->identity_column, $identity)
		                  ->or_where('scout_id', $identity)
		                  ->limit(1)
