...
class Portal extends REST_Controller {
   ...
   public function verify_scout_id_get(){
      $scoutID = strtoupper($this->get('scoutID'));

      $results = $this->Api_model->find_scout_id($scoutID);
      if($results){
         $this->response(array('status'=> 'true', 'message' => 'success', 'result'  => $results), REST_Controller::HTTP_OK); 
      }else{
         $this->response(array('status' => 'false', 'message' => 'No data fount', 'result' => ''), REST_Controller::HTTP_OK);
      }
   }
